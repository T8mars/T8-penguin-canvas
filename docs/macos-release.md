...
v3.0.0 的 Windows Tag 已固定在旧源码提交。Mac 构建支持是在发布后补入的，因此只允许创建透明的递增辅助源码 Tag，不得移动 `v3.0.0`。`v3.0.0-mac.1` 的首轮实机构建正确拦截了不完整的私有前端恢复，未上传任何资产；修复后的固定源码使用 `v3.0.0-mac.2`，不得移动或复用失败 Tag：

1. 在核心目录完成代码、测试、文档与 `features.json`，提交并推送 `origin/main`。
2. 将新的递增辅助 Tag（当前为 `v3.0.0-mac.2`）固定到该提交并推送；每次修复都增加序号，禁止移动旧 Tag。
