...
更新：2026-09-22。仅维护当前事实；长期规则见根 `SKILL.md` 和 `AGENTS.md`。本页预算 80 行 / 8 KiB，旧检查点迁入专题，不追加长日报。
...
- package 已按十进制展示序列升级为 `3.2.0`（禁止 `3.1.10`），固定范围为 Qwen Image Global 2.1 / Animate Motion Transfer 双独立 Tab、完整官方参数、恢复/结果落盘与四份无凭据工作流；当前是正式发布已授权、技术链待执行状态。详情见[发布专题](release-v3.2.0.md)。
- v3.2.0 真实受影响用户、安装环境及 F8–F10 证据按 `owner-approved-post-release-v3.2.0` 后补且不视为通过；正式技术门、固定源码、Windows 自动更新与完整回下载不可延期，Mac 仅在 GitHub 额度真实不足时可延期。
...
| v3.2.0 发布 | 已授权，技术链待执行：Windows 必须完成唯一正式 Electron/NSIS、provenance、sealed recovery、自动更新三资产和远端完整下载；Mac 应从同一 `v3.2.0` Tag 运行，只有 GitHub 额度实际不足时可延期。不预写 commit、资产、时间或 workflow 通过。 |
| Qwen 2.1 / Animate | 开发与真实 Provider 验证完成，已授权纳入 v3.2.0：图像/视频节点独立 Tab、精确参数、恢复、可信结果落盘和四份无凭据工作流已接入。T2I、I2I、Animate 三任务均到达 `succeeded`，Animate 产物完成下载解析；Qwen 当次结果下载受本机 Node TLS 中断，未重交付费任务。详见[专题](qwen21-animate-motion-transfer.md)和 `feature qwen21AnimateMotionTransfer20260922`。 |
